...
class WhiskyDataEngine:
    ...
    def get_distillery_metrics(slug):
        ...
        if not df.is_empty():
            return df.with_columns(pl.col("dt").str.to_date("%Y-%m-%d").alias("Timestamp"))
        return pl.DataFrame()
...
class AnalyticsUI:
    ...
    def render_top_lists(df):
        ...
        # Aggregation Logic
        top_volume = (df.group_by("distillery_slug")
                   .agg((pl.col("trading_volume").sum()/1000000).alias("Total Volume Millions"))
                   .rename({"distillery_slug": "Distillery"})
                   .sort("Total Volume Millions", descending=True).head(10))
        
        top_revenue = (df.group_by("distillery_slug")
                   .agg([
    ...
    def render_deep_dive(df, name):
        ...
        with c1:
            ...
            fig_mean = px.line(df.to_pandas(), x="Timestamp", y="winning_bid_mean", template="plotly_white")
            fig_mean.update_traces(line_color='#D4AF37')
            st.plotly_chart(fig_mean, use_container_width=True)
        with c2:
            st.subheader("Auction Velocity (Lots)")
            fig_lots = px.bar(df.to_pandas(), x="Timestamp", y="lots_count", template="plotly_white")
        ...
        fig_spread.add_trace(go.Scatter(
            x=df_pd['Timestamp'], y=df_pd['winning_bid_max'],
            name='Market Ceiling',
            line=dict(color='#D4AF37', width=1)
        ))
        # Min Price with Shaded Area
        fig_spread.add_trace(go.Scatter(
            x=df_pd['Timestamp'], y=df_pd['winning_bid_min'],
            name='Market Floor',
            fill='tonexty',
...
def main():
    ...
    try:
        ...
        with tab_deepdive:
            ...
            with col_search:
                selected_slug = st.selectbox("Select Distillery for Analysis", options=slug_list)
